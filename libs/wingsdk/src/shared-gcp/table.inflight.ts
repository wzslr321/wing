import { Bigtable } from "@google-cloud/bigtable";
 ...
 export class TableClient implements ITableClient {
 	...
 	public async update(key: string, row: Json): Promise<void> {
 		log(`method invoked for ${this.tableName} ${this.primaryKey} ${this.columns}`);// lol
 	...
 	public async upsert(_key: string, _row: Json): Promise<void> {
 		throw new Error("Method not implemented.");
 	}
 	public async tryGet(_key: string): Promise<Json | undefined> {
 		throw new Error("Method not implemented.");
 	}
 }
